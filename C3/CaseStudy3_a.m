...
A =  load('graph3.mat');% Adjacency matrix
A = table2array(struct2table(A)) + eye(n);  % add diagonal to A
beta = 1*rand(n,n);     % Infection rate \beta_{ij}
...
dt = 0.01/2;    % Numerical integration time step
tmax = 50;    % Time range
...
for k=1:length(vector_c)
    ...
    c = vector_c(k)
    fbr_in = @(y,yglobal) c*(y.^2) + (1-c)*(yglobal.^2);   % Link-breaking within a community
    fcr_in = @(y,yglobal) (1-y.*y);          % Link-creation within a community
    fbr_out = @(y1,y2,yglobal) y1.*y2;   % Link-breaking between communities
