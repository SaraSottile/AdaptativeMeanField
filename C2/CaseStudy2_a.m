...
A =  load('graph2.mat');% Adjacency matrix
A = table2array(struct2table(A)) + eye(n);       % add diagonal to A
beta = 0.25*rand(n,n);     % Infection rate \beta_{ij}
...
z_init = 1*ones(n,n);                   % initial links
...
for k=1:length(vector_c)
    ...
    c = vector_c(k)
    fbr_in = @(y,yglobal) c*(y.^2);   % Link-breaking within a community
    fcr_in = @(y,yglobal) (1-y.*y);          % Link-creation within a community
    fbr_out = @(y1,y2,yglobal) 1/c*y1.*y2;   % Link-breaking between communities
...
figure; set(gcf,'position',[200 200 400 300]); hold all;
plot(vector_c, vector_y_peak, '<-','Linewidth',2,'DisplayName','peak','Color', [0.6350, 0.0780, 0.1840]);
plot(vector_c, vector_y_ss, 'o-','Linewidth',2,'DisplayName','steady-state','Color', [0.4660, 0.6740, 0.1880]);
hold off;
box on;
set(gca, 'XScale', 'log');
legend('Interpreter','latex');
